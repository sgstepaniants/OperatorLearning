...
class fokker_planck:
    def __init__(self, grid, temperature, drag,
                 boundary=fplanck.boundary.reflecting, potential=None, force=None):
        """
        ...
        self.Ngrid = self.grid[0].shape
        
        self.grid_stack = np.stack(self.grid, axis=-1)
        self.resolution = [1] if self.grid[0].size == 1 else self.grid_stack[(1,)*self.ndim + (slice(None),)] - self.grid_stack[(0,)*self.ndim + (slice(None),)]
