...
train_errors = zeros(1, numalphas);
test_errors = zeros(1, numalphas);
...
for s = 1:numalphas
    ...
    for k = 1:fullsize
        currheatpde =@(x, t, u, dudx) heatpde(x, t, u, dudx, alpha);
        heatic =@(x) fs(max(ceil(m*x/L), 1), k);
        sol = pdepe(0, currheatpde, heatic, @heatbc, x, t);
        sols(:, :, k) = sol';
    end
    ...
    train_errors(s) = mean(l2norm(us_train_hat - us_train) ./ l2norm(us_train));

    % compute the mean relative test error
    us_test_hat = repmat(nu, [1, testsize]) + h * G_hat * fs_test;
    test_errors(s) = mean(l2norm(us_test_hat - us_test) ./ l2norm(us_test));
end

plot(alphas, train_errors)
hold on
plot(alphas, test_errors)
xlabel('\alpha')
legend('train error', 'test error')
title('Train and Test Errors vs. Nonlinearity')
